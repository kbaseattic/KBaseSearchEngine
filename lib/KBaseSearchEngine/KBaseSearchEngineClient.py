...
class KBaseSearchEngine(object):
    ...
    def search_objects(self, params, context=None):
        ...
        :returns: instance of type "SearchObjectsOutput" (Output results for
           ...
           "ws_owner_displayname" of String, parameter "workspaces_info" of
           mapping from type "access_group_id" (A data source access group ID
           (for instance, the integer ID of a workspace).) to type
           ...
           workspace.) -> tuple of size 9: parameter "id" of type "ws_id"
           ...
           time)), parameter "max_objid" of Long, parameter "user_permission"
           ...
           "globalread" of type "permission" (Represents the permissions a
           user or users have to a workspace: 'a' - administrator. All
           operations allowed. 'w' - read/write. 'r' - read. 'n' - no
           permissions.), parameter "lockstat" of type "lock_status" (The
           lock status of a workspace. One of 'unlocked', 'locked', or
           'published'.), parameter "metadata" of type "usermeta" (User
           provided metadata about an object. Arbitrary key-value pairs
           provided by the user.) -> mapping from String to String, parameter
           "objects_info" of mapping from type "obj_ref" (An X/Y/Z style
           reference) to type "object_info" (Information about an object,
    ...
    def get_objects(self, params, context=None):
        ...
        :returns: instance of type "GetObjectsOutput" (Output results of
           ...
           value returned by get_object_info3() for each object in the search
           results is saved in the mapping.) -> structure: parameter
           "objects" of list of type "ObjectData" (Properties of an object
           including metadata, raw data and keywords. GUID guid - the
           object's guid. GUID parent_guid - the guid of the object's parent
           if the object is a subobject (e.g. features for genomes).
           object_name - the object's name. timestamp - the creation date for
           the object in milliseconds since the epoch. string type - the type
           of the data in the search index. int type_ver - the version of the
           search type. string creator - the username of the user that
           created that data. string copier - if this instance of the data is
           a copy, the username of the user that copied the data. string mod
           - the name of the KBase SDK module that was used to create the
           data. string method - the name of the method in the KBase SDK
           module that was used to create the data. string module_ver - the
           version of the KBase SDK module that was used to create the data.
           string commit - the version control commit hash of the KBase SDK
           module that was used to create the data. parent_data - raw data
           extracted from the subobject's parent object. The data contents
           will vary from object to object. Null if the object is not a
           subobject. data - raw data extracted from the object. The data
           contents will vary from object to object. key_props - keyword
           properties of the object. These fields have been extracted from
           the object and possibly transformed based on the search
           specification for the object. The contents will vary from object
           to object. mapping<string, list<string>> highlight - The keys are
